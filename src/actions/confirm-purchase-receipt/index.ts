...
import { confirmPurchaseReceiptSchema } from "./schema";

export const confirmPurchaseReceiptAction = actionClient
  .schema(confirmPurchaseReceiptSchema)
  .action(async ({ parsedInput }) => {
    ...
    // Se não estava "recebida" antes, adicionar a quantidade ao estoque
    if (
      ...
      if (inventory) {
        ...
        await db
          ...
          .set({
            ...
            lastPurchase: parsedInput.receiptDate,
            updatedAt: new Date(),
          })
          .where(eq(inventoryTable.id, currentPurchase.inventoryId));
    ...
    // Atualizar a compra com os dados de recebimento
    await db
      .update(purchasesTable)
      .set({
        status: "recebida",
        deliveryDate: parsedInput.receiptDate,
        receiverName: parsedInput.receiverName,
